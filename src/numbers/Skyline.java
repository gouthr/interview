...
public class Skyline {
    public List<int[]> getSkyline(int[][] buildings) {
    	/** Inefficient solution using lots of memory. Will definitely run out of memory if the bounds are large. */
        List<int[]> res = new ArrayList<>();
        if (buildings == null || buildings.length == 0 || buildings[0].length != 3) {
            return res;
        }
        
        int[] heights = new int[Integer.MAX_VALUE];
        
        int m = buildings.length;
        for (int i=0; i<m; i++) {
            for (int k=buildings[i][0]; k<buildings[i][1]; k++) {
                if (buildings[i][2] > heights[k]) {
                    heights[k] = buildings[i][2];
                }
            }
        }
